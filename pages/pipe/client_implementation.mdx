...
import {getPackageDetails} from '../../utils/npm';

export const getStaticProps = async () => {
	const [client, react] = await Promise.all([
		getPackageDetails('@onehop/client'),
		getPackageDetails('@onehop/react'),
	]);

	return {
		props: {
			ssg: {
				client: client['dist-tags'].latest,
				react: client['dist-tags'].latest,
			},
		},
...
export const InstallationWithVersion = () => {
	const ssg = useSSG();

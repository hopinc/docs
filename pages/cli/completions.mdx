...
description: Add tab completion to the Hop CLI
---
...
<Callout>
	If you get `permission denied` error, you can try running the following
	command:
</Callout>
...
Completions typically will be installed automatically by your shell's package manager and the script will _try_ to install them.
...
If you have [`bash-completion`](https://github.com/scop/bash-completion) installed, you can just copy your new completion script to the `/usr/local/etc/bash_completion.d` or `/usr/share/bash-completion/completions` directory using the following command:

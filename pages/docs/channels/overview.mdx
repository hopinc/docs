...
Channels can be given the `no_auth` capability, which means that you don't need to worry about handing out tokens to users on your frontend. This is useful if you want to display realtime data on a frontend application that doesn't require user sign-in, for example.
...
A channel subscription refers to a client tokens being subscribed to a channel.

**Important:** If you are just subscribing to unprotected (`no_auth`) channels on your frontend, then all channel subscriptions are ephemeral and last the duration of the socket connection.

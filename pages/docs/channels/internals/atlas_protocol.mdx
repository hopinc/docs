...
The CAP theorem states that in the event of a network partition, you can only favor either **availability** _or_ **consistency**, but not both. In our case, we need to favor availability for the ability to look up a resource and understand if the response is inconsistent or not.

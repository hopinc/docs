...
We're effectively partitioning the CAP theorem's guarantees and using the _availability_ we get from one service (Redis), and then pairing it with a service acting as a _consistent_ layer (individual Elixir nodes) by optimistcally taking ownership of resources if a partition is observed.

...
| field      | type    | description                                       |
| ---------- | ------- | ------------------------------------------------- |
| pipe_room   | [pipe_room](/docs/pipe/api_reference/room) | the pipe room object, with sensitive fields stripped |
| connection | ?ConnectionMap* | map connection objects, documented below |

*connection will be `null` if stream is `offline` (pipe_room:state)
